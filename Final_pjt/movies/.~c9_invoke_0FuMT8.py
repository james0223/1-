...
def list(request):
    ...
    if request.user.is_authenticated:
        person = request.user
        fav = person.love_set.all().order_by('-value')[:3]
        fav_one = fav[0].genre
        fav_two = fav[1].genre
        fav_thr = fav[2].genre
        foru1 = Movie.objects.all().filter(genres=fav_one).order_by('-vote_average')[:5]
        foru2 = Movie.objects.all().filter(genres=fav_two).order_by('-vote_average')[:5]
        foru3 = Movie.objects.all().filter(genres=fav_thr).order_by('-vote_average')[:5]
        context = {
            'top1':top1,
            'top2':top2,
            'top3':top3,
...
def now_playing(request):
    movies = movies = Movie.objects.filter(now_playing=True)
    context = {
        'movies' : movies,
    }
...
def upcoming(request):
    movies = movies = Movie.objects.filter(upcoming=True)
    context = {
        'movies' : movies,
    }
    return render(request, 'movies/nowPlaying.html', context)
...
def search(request):
    ...
    if search:
        ...
        if resMovies:
            Sum = resMovies.count()
            for i in range(Sum):
                res.append({'title': resMovies[i].title, 'id':resMovies[i].id, 'backdrop_path':resMovies[i].backdrop_path})
        context = {
            'movies':movies,
            'search':search,
            'res':res,
...
def movie_detail(request,movie_pk):
    ...
    lst=''
    if request.user.is_authenticated:
        find = request.user.moviecomment_set.all().filter(movie_id = movie_pk).count()
        lst = request.user.moviecomment_set.all()
        ...
        if find > 0:
            usercomment = request.user.moviecomment_set.all()
    context = {
        'movie':movie,
        'form':form,
        'comments':comments,
